...
def _make_point_geojson(lon: float, lat: float) -> str:
    """Build GeoJSON Point string from lon/lat."""
    return json.dumps({"type": "Point", "coordinates": [lon, lat]})


def _make_geometry_json(geom: dict) -> str | None:
    """Convert GeoJSON geometry dict to JSON string."""
    if not geom:
        return None
...
async def seed(session: AsyncSession, profile: dict) -> None:
    ...
    for launch_data in profile.get("launches", []):
        ...
        point_json = _make_point_geojson(launch_data["lon"], launch_data["lat"])
        launch = Launch(
            site_id=site_id,
            name=launch_data["name"],
            point_geom=point_json,
    ...
    for lz_data in profile.get("landings", []):
        ...
        point_json = _make_point_geojson(lz_data["lon"], lz_data["lat"])
        landing = Landing(
            site_id=site_id,
            name=lz_data["name"],
            point_geom=point_json,
    ...
    for tf_data in profile.get("terrain_features", []):
        ...
        geom_wkt = _make_geometry_json(tf_data.get("geometry"))
